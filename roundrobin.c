...
void ExecuteProcess() {
    if (gpCurrentProcess->mRuntime == gpCurrentProcess->mRemainTime) { //if this process never ran before
        ...
        if (!gpCurrentProcess->mPid) { //if child => execute the process
...
void AlarmHandler(int signum) {
    gpCurrentProcess->mRemainTime -= gQuanta; //decrease the remaining time by the quanta
    ...
    if (kill(gpCurrentProcess->mPid, SIGTSTP) == -1) { //stop current process
		...
		//printf("RR: *** retry to stop the process");
...
int HandleTie() {
    ...
    while (pTemp->mArrivalTime == gpCurrentProcess->mArrivalTime) { //loop to identfy whether there is a tie or not
	//tie is that the current process and next process shares the same arrival time
